...
void deallocateList(LLNode* root)
{
    if(root->payP == (Payload*)0)
    {
        printf("Nothing to deallocate!\n");
    }


    else if (root->next == (struct LLNode*)0)
    {
        free(root);
    }
    else {
        deallocateList((LLNode*)root->next);
        free(root);
    }
    return;
...
void printPath(LLNode* nodeP) {
    if(nodeP->payP == (Payload*)0)
    {
        printf("Path length is 0\n");
    }


    else if (nodeP->next == (struct LLNode*)0)
    {
        Marker* m = (Marker*) nodeP->payP;
